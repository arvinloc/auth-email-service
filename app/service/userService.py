...
class UserService:
    ...
    def signup(self, user_details: UserInCreate) -> UserOutput:
        if self.__userRepository.user_exist_by_email(email=user_details.email):
            raise HTTPException(
                status_code=400, detail="Unable to register with these credentials")
    ...
    def issue_magic_link(self, user: UserOutput) -> None:
        raw_token = secrets.token_urlsafe(32)  # создание токена

        token_hash = hashlib.sha256(
            raw_token.encode()).hexdigest()  # хеширование
        # установка ttl
        expires_at = datetime.now(timezone.utc) + \
            timedelta(minutes=MAGIC_LINK_TTL_MINUTES)

        # создагте пейлоада с токеном
    ...
    def consume_magic_link(self, raw_token: str):
        ...
        token_record = self.__magicLinkRepository.mark_used_if_unused(
            token_hash=token_hash)
        ...
        if not user.is_verified:  # проверка на верификацию
